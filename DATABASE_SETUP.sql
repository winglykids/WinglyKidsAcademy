...
alter table public.students add column if not exists payment_received_at timestamptz;
alter table public.students add column if not exists payment_updated_at timestamptz;
...
alter table public.homework_assignments add column if not exists reviewed_at timestamptz;
alter table public.homework_assignments add column if not exists attachment_storage_path text;
alter table public.homework_assignments add column if not exists attachment_filename text;
alter table public.homework_assignments add column if not exists attachment_file_type text;
alter table public.homework_assignments add column if not exists attachment_file_size bigint;
...
create table if not exists public.payment_reminder_logs (
  id uuid primary key default gen_random_uuid(),
  lesson_id uuid references public.lesson_bookings(id) on delete cascade,
  student_id uuid references public.students(id) on delete cascade,
  reminder_type text not null default 'payment_due_tomorrow',
...
alter table public.messages add column if not exists assignment_id uuid references public.homework_assignments(id) on delete set null;
...
drop policy if exists "Teachers can read payment reminder logs" on public.payment_reminder_logs;
create policy "Teachers can read payment reminder logs"
...
grant select on public.payment_reminder_logs to authenticated;
